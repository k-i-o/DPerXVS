...
int main(int, char**) {
    ...
    LPCSTR convertedDDPERClient = CT2A(DDPERClient);
    LPCSTR convertedDDNetClient = CT2A(DDNetClient);

    GameInfo gameInfo;
    gameInfo.Initialize(client == GameInfo::DDPER ? convertedDDPERClient : convertedDDNetClient, client == GameInfo::DDPER ? "DDPER.exe" : "DDNet.exe");
    ...
    std::cout << ("DPerX VS edition by kio opened successfully!") << std::endl;
